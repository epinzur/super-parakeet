    ...
    with open(jsonl_file_path, 'r') as file:
        with tqdm(total=total_lines, desc="Reading lines") as pbar:
            ...
            while line:
                ...
                try:
                    title = json_object["title"].encode('utf-8').decode('unicode_escape')
                except Exception:
                    title = json_object["title"]

                try:
                    text = json_object["snippet"].encode('utf-8').decode('unicode_escape')
                except Exception:
                    text = json_object["snippet"]
...
if __name__ == "__main__":
    load_and_insert_chunks("datasets/complexwebquestions_V1_1/web_snippets.jsonl", dry_run=False)
